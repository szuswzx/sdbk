...
include('Weixin_BaseCore.php');
include('Weixin_Template.php');
...
class Weixin{
	...
	public function __construct($config = array('AppId' => '', 'AppSecret' => '')){

		$this->app_id = $config['AppId'];
		$this->app_sercet = $config['AppSecret'];
...
//have appkey and appsercet
	...
	public function pushtemple($token, $open_id, $template_id, $templeurl, $infos){
		return $this->base->pushtemple($token, $open_id, $template_id, $templeurl, $infos);
	}
///END PUSH
...
//uplaod and download
	...
	public function uploadImgUrl($token, $filename){
		$res = $this->base->uploadImgUrl($token, $filename);
		if(isset($res['url']))
			return $res['url'];
		else
			return '';
