...
class Weixin_BaseCore{
	...
	* @ret string $response
	...
	public function pushMsgNews($token, $open_id, $infos){
		$info['touser'] = $open_id;
		$info['msgtype'] = 'news';
		$info['news']['articles'] = $infos;
		$url = "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token={$token}";
		return $this->get($url, $this->to_json($info));
	}
	...
	public function uploadImgUrl($token, $filename){
		$info = array("media" => new CURLFile(realpath($filename)));;
		$url = "https://api.weixin.qq.com/cgi-bin/media/uploadimg?access_token={$token}";
		return json_decode($this->get($url, $info), true);
	}
